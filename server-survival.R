output$out_surv__pictograph = renderPlot({
    ...
    pictograph_icons = get_pictograph_icons(category_colors = colors, deceased_color = list(CONST$empty) %>% set_names(dutch$UI$deceased))
    picto = if(length(survival)==0) {
      surv = list(EMPTY=100)
      plot_pictograph(surv,pictograph_icons,from_top = T,picto_title="", show_legend = FALSE, wide = is_wide)
    } else {
      ...
      if(length(surv)<=2) {
        ...
        plot_pictograph(surv,pictograph_icons, from_top = T,picto_title=plot_title,wide=is_wide)
        ...
        sub_pictos = lapply(1:length(treatments),function(i) plot_pictograph(surv[[i]] %>% set_names(treatments[[i]]),pictograph_icons,from_top = T,
                                                                             picto_title=treatments[i],wide=length(treatments)==3, show_legend = FALSE))
        plot_title = textGrob(dutch$PLOT$time0(n_months), gp=gpar(fontface="bold",fontsize=CONST$size_title))
        grid = if(length(survival)==3) list(rows=3, columns=1) else get_square_grid(length(survival))
        arrangeGrob(grobs=sub_pictos, ncol=grid$rows, nrow = grid$cols, top=plot_title)
      }
    }
...
output$out_surv__km = renderPlotly({
    ...
    p = if(!draw_nothing) {normal+geom_line(size=1)} else {empty+geom_blank()}
           ...
           scale_x_continuous(name ="Tijd na diagnose (in maanden)" , limits = c(0, n_months*CONST$month),
           ...
           scale_fill_manual(name="",values=unlist(colors)) + scale_colour_manual(name="",values=unlist(colors)) +
             theme(legend.position = "top",panel.border = element_rect(fill=NA,colour="black",size=1),
                   ...
                   plot.title=element_text(size=CONST$size_title,vjust=-5),
                   text = element_text(size = CONST$size_text),
                   axis.title.x = element_text(vjust=-2, size=CONST$size_title))+
           ###Confidence interval gebied om de lijn
           if(input$set_surv_conf & !draw_nothing) geom_ribbon(data=surv, aes(x=time, ymax=upper, ymin=lower, group =label, fill=label, color=label),
                               alpha=0.2,position = "identity",inherit.aes = F)

    if("mid" %in% input$set_percentile && !draw_nothing  && nrow(surv[surv$surv >=0.25 & surv$surv<=0.75,]) > 0)
    {
    ...
    if("bad" %in% input$set_percentile && !draw_nothing && nrow(surv[surv$surv >=0.75,]) > 0) {
     ...
     if("good" %in% input$set_percentile && !draw_nothing && nrow(surv[surv$surv <= 0.25,]) > 0) {
